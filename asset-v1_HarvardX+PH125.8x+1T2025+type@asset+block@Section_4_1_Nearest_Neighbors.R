...
# Section 4: Cross-validation and k-Nearest neighbours

## Section 4.1 Nearest neighbours
...
p1 <- mnist_27$true_p %>% 
  ...
  ggplot() +
  geom_point(data = mnist_27$train, aes(x_1, x_2, colour= y), pch=21) +
  scale_fill_gradientn(colours=c("#F8766D", "white", "#00BFC4")) +
  stat_contour(aes(x_1, x_2, z = knn), breaks=c(0.5), colour="black") +
  ggtitle("Train set")
p2 <- mnist_27$true_p %>% 
  mutate(knn = predict(knn_fit_1, newdata = .)[,2]) %>%
  ggplot() +
  geom_point(data = mnist_27$test, aes(x_1, x_2, colour= y), 
